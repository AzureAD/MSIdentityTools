...
param (
    ...
    [string]$Path,
    [Parameter(Mandatory = $true)]
    [ValidateSet("All", "PrincipalName", "RFC822Name", "IssuerAndSubject", "Subject", "SKI", "SHA1PublicKey", "IssuerAndSerialNumber")]
...
try {
    ...
    if ($CertificateMapping -eq "All")
    {
        foreach ($property in $mappings.GetEnumerator()) {
            Write-Host "`n$($property.Key):"
            if ([string]::IsNullOrEmpty($property.Value)) {
                Write-Host "mapping not available for this certificate" -ForegroundColor Yellow
            } else {
    ...
    else
    {
        ...
        Write-Host "$($CertificateMapping): $($value)"
    }
} catch {
