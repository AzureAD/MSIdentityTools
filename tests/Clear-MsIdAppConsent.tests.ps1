...
Describe 'Clear-MsIdAppConsent' {
    ...
    BeforeAll {
        ...
        InModuleScope $PSModule.Name {
            ...
            if (!(Get-Command Get-MgServicePrincipalOauth2PermissionGrant -ErrorAction SilentlyContinue)) {
                function script:Get-MgServicePrincipalOauth2PermissionGrant ($Filter) { throw "The term '$($MyInvocation.MyCommand)' is not recognized as the name of a cmdlet, function, or operable program and there is no mock defined." }
            }
            if (!(Get-Command Remove-MgOauth2PermissionGrant -ErrorAction SilentlyContinue)) {
                function script:Remove-MgOauth2PermissionGrant ($Filter) { throw "The term '$($MyInvocation.MyCommand)' is not recognized as the name of a cmdlet, function, or operable program and there is no mock defined." }
            }
        }
        ...
        $TestCases = @(
            ...
            @{ ClientId = '20000000-0000-0000-0000-000000000002'; TenantWideAdminConsent = $true }
            @{ ClientId = '20000000-0000-0000-0000-000000000002'; PrincipalId = '50000000-0000-0000-0000-000000000005'; TenantWideAdminConsent = $true }
    ...
    Context 'ClientId: <ClientId>' -ForEach @(
        ...
        It 'Positional Parameter' {
            $Output = Clear-MsIdAppConsent $ClientId @params -ErrorVariable actualErrors
        ...
        It 'Pipeline Input' {
            $Output = $ClientId | Clear-MsIdAppConsent @params -ErrorVariable actualErrors
            $Output | Should -BeNullOrEmpty
            Should -Invoke Test-MgCommandPrerequisites -ModuleName $PSModule.Name
            Should -Invoke Get-MgServicePrincipal -ModuleName $PSModule.Name -ParameterFilter {
                $Filter -eq "appId eq '$ClientId'"
    ...
    Context 'Multiple Input' {
        It 'Positional Parameter' {
            $Output = Clear-MsIdAppConsent $TestCases.ClientId -All -ErrorVariable actualErrors
            $Output | Should -BeNullOrEmpty
            Should -Invoke Test-MgCommandPrerequisites -ModuleName $PSModule.Name
        ...
        It 'Pipeline Input' {
            $Output = $TestCases.ClientId | Clear-MsIdAppConsent -All -ErrorVariable actualErrors
            $Output | Should -BeNullOrEmpty
            Should -Invoke Test-MgCommandPrerequisites -ModuleName $PSModule.Name
            for ($i = 0; $i -lt $TestCases.Count; $i++) {
                Should -Invoke Get-MgServicePrincipal -ModuleName $PSModule.Name -ParameterFilter {
